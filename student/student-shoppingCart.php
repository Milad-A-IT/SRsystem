...
//$lname = $row['last_name'];
  ...
  $csql = "SELECT c.course_id AS id, c.title as coursename, c.unit as unit, c.course_lecturer as lecturer, c.location as location, c.time as classTime FROM courses c, cart r WHERE r.sid = '$id' AND c.course_id = r.course_id";
...
<html lang="en">
<head>
    <title>GROUP 404 | Shopping Cart</title>
    <meta charset="utf-8">
          ...
          <thead>
            ...
            <tfoot>
              <tr>
                  <th></th>           
                  <th></th>
                  <th colspan="2" style="text-align:right">Total Credit: <?php echo $sum; ?> credits</th>
                  <th></th>
              </tr>
            </tfoot>
            </table>
            <br>
		        <br>
            <button type="button" class="btn btn-warning" name="btn_enroll" id="btn_enroll">Enroll</button>	
            <button type="button" class="btn btn-danger" name="btn_drop" id="btn_drop">Drop</button>	
      </div>
    </div>
    <?php
      }
          ...
          <thead>
            ...
            <tfoot>
              <tr>
                  <th></th>           
                  <th></th>
                  <th colspan="2" style="text-align:right">Total Credit: 0 credits</th>
                  <th></th>
              </tr>
            </tfoot>
              </table>
...
else if(confirm("Confirm to enroll?"))
{
    $.post("scripts/student-registered.php",
    {
      "id" : id
    },
		function(result){
		  if($.trim(result) == "error"){
			  alert("Fail to enroll");
	  	} else {
			  alert("Enroll Successfull");
		  }
      document.location="student-shoppingCart.php";
    }
    );
}
else
...
else if(confirm("Confirm to drop?"))
{
    $.post("scripts/student-dropCourse_sc.php",
    {
      "id" : id
    },
		function(result){
