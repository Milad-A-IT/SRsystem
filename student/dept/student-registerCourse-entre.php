...
$row = mysqli_fetch_assoc($result);
  ...
  $csql = "SELECT c.course_id as id, c.title as coursename, c.unit as unit, c.course_lecturer as lecturer, c.location as location, c.semester as semester, c.time as classTime FROM courses c, department d WHERE c.course_id NOT IN ( SELECT course_id FROM cart WHERE sid='$id' ) AND c.course_id NOT IN ( SELECT course_id FROM course_registered WHERE sid='$id') AND d.dept_name = 'ENTREPRENEURSHIP' AND c.department = d.dept_id AND c.semester = 'FALL'";
  ...
  $.post("../scripts/student-registerCourse_check.php",
    {
      ...
      document.location="student-registerCourse-entre.php";
    }
    );
}
else
