...
$cresult = mysqli_query($db,"SELECT c.course_id as id, c.title as coursename, c.unit as unit, c.course_lecturer as lecturer, d.dept_name as department, c.semester as semester, c.time as classTime, c.location as location FROM courses c, department d WHERE c.department = d.dept_id");

?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>GROUP 404 | All Courses</title>
    <meta charset="utf-8">
          ...
          <thead>
            ...
            ?>
                <tr id="<?php echo $crow["id"]; ?>]">
                    <td><input type="checkbox" name="course_id[]" class="viewcourse" value="<?php echo $crow["id"];?>"/></td>
                    <td><?php echo $crow["id"];?></td>
                    <td><?php echo $crow["coursename"];?></td>
                    <td><?php echo $crow["unit"];?></td>
                    <td><?php echo $crow["lecturer"];?></td>
                    <td><?php echo $crow["department"];?></td>
...
{
    $.post("scripts/admin-deleteCourse.php",
    {
      "id" : id
    },
		function(result){
      ...
      document.location="admin-viewCourse.php";
    }
    );
}
else
...
{
  $.ajax({
      url:'admin-updateCourse.php',
      method:'POST',
      data:{id:id},
      success:function(data)
      {
        window.location = "admin-updateCourse.php?id="+id;
      }
